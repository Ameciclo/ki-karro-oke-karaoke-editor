...
{
    qint64 timing = editor->goToPreviousTimingTag( m_mediaPlayer ? m_mediaPlayer->position() : 0 );

    if ( timing != -1 )
    {
        playerSeekToTime( timing );

        if ( m_mediaPlayer && m_mediaPlayer->isMediaReady() )
        {
            m_mediaPlayer->play();
...
{
    qint64 timing = editor->goToNextTimingTag( m_mediaPlayer ? m_mediaPlayer->position() : 0 );
